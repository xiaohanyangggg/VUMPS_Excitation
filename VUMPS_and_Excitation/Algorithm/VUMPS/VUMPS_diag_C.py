...
def update_env_Arnoldi(psi, H, old_env_data, Krylov_Para):
    MPO_TM_AR = MPOTransferMatrix(H, psi, transpose=False)
    MPO_TM_AL = MPOTransferMatrix(H, psi, transpose=True)
    P_tol = Krylov_Para["P_tol"]
    E_tol = Krylov_Para["E_tol"]
...
def run_parallel_uMPS(H, psi, N_2Site=10, method='polar', tol=1e-12, max_iter_num=300, checkpoint_num=500, Krylov_Para={'E_tol':1.e-13, 
    ...
    while eps_prec >= tol and n_iter < max_iter_num:
        ...
        for i in range(0, L):
            ...
            # print("E0_Ac is:{}".format(E0_Ac))
            E0_CL, CL_Til, N_CL = Gs_CL.run()
            # print("E0_C is:{}".format(E0_CL))
            psi.set_Bc(i, Ac_Til.ireplace_label('p0', 'p'))
            psi.set_C(i, CL_Til)
            # print("Updated Bc is: {}".format(psi._Bc[i]))

        """ SVD each C tensor to fix diagonal gauge for local tensors """
        for i in range(0, L):
            U_C, S_i, VH_C = npc.svd(psi._C[i], 
                                     cutoff=None,
                                     qtotal_LR=[psi._C[i].qtotal, None],
                                     inner_labels=['vR', 'vL'])
            if i == 0:
                Left_U = U_C
                Right_U = VH_C
            psi.set_SL(i, S_i)
            # print("Singular values of SVD:{}".format(S_i))
            # If S_i has 0s, just add them by hand
            if len(S_i) < U_C.get_leg(1).ind_len:
                S_zeros = np.array([0]*(U_C.get_leg('vR').ind_len-len(S_i)))
                S_i = np.concatenate((S_i, S_zeros), axis=0)
            psi.set_C(i, npc.diag(S_i, leg=U_C.get_leg('vR').conj(), labels=['vL', 'vR']))
            if L == 1:
                Ac_t = npc.tensordot(psi.get_Bc(0), VH_C.conj(), axes=['vR', 'vR*']).replace_label('vL*', 'vR').transpose(['vL', 'p', 'vR'])
                Ac_t = npc.tensordot(Ac_t, U_C.conj(), axes=['vL', 'vL*']).replace_label('vR*', 'vL').transpose(['vL', 'p', 'vR'])
                psi.set_Bc(0, Ac_t)
            else:
                Ac_L_t = npc.tensordot(psi.get_Bc(i-1), VH_C.conj(), axes=['vR', 'vR*']).replace_label('vL*', 'vR').transpose(['vL', 'p', 'vR'])
                Ac_R_t = npc.tensordot(psi.get_Bc(i), U_C.conj(), axes=['vL', 'vL*']).replace_label('vR*', 'vL').transpose(['vL', 'p', 'vR'])
                psi.set_Bc(i-1, Ac_L_t)
                psi.set_Bc(i, Ac_R_t)

        """ for i in range(0, L):
            print("C[{}]: \n".format(i), psi.get_C(i))
            print("Ac[{}]: \n".format(i), psi.get_Bc(i)) """
        S0 = psi.get_SL(0)
        psi._S[L] = S0
        ...
        for i in range(0, L):
            ...
            if method == "svd":
                ...
                # eps_L = npc.norm(psi.get_Bc(psi._to_valid_index(i-1)) - npc.tensordot(A_L, psi.get_C(i), axes=['vR', 'vL']))
                # print("eps_L is:{}".format(eps_L))
                # eps_R = npc.norm(psi.get_Bc(i) - npc.tensordot(psi.get_C(i), A_R, axes=['vR', 'vL']))
                # print("eps_R is:{}".format(eps_R))
        ...
        E_n = H.expectation_value(psi)
        print("Energy is:{}".format(E_n))
        Energy.append(E_n)
